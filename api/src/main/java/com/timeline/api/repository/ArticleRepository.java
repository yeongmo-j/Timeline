...
public interface ArticleRepository extends CrudRepository<ArticleEntity, Integer>{
	...
	List<ArticleEntity> getTimeline(int userID);
	
	@Query(value = "SELECT * FROM ARTICLE WHERE id = ?1", nativeQuery=true)
	ArticleEntity findArticle(int articleID);
	
	@Query(value="SELECT * FROM ARTICLE WHERE ARTICLE.userid=?1 ORDER BY ARTICLE.id DESC", nativeQuery=true)
	List<ArticleEntity> getHome(int userID);
